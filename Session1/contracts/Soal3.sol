...
contract BasedBadge is ERC1155, AccessControl, Pausable, ERC1155Supply {
    ...
    function createBadgeType(
        ...
        } else if (keccak256(bytes(category)) == keccak256("EVENT")) {
    ...
    function issueBadge(address to, uint256 tokenId) public onlyRole(MINTER_ROLE) {
        ...
        require(totalSupply(tokenId) + 1 <= tokenInfo[tokenId].maxSupply, "Max supply reached");
        ...
        public onlyRole(MINTER_ROLE)
    {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            require(totalSupply(tokenId) + amount <= tokenInfo[tokenId].maxSupply, "Max supply reached");
        ...
        public view returns (bool valid, uint256 earnedTimestamp)
    {
        ...
        if (tokenInfo[tokenId].validUntil != 0 && block.timestamp > tokenInfo[tokenId].validUntil) {
            return (false, earnedAt[tokenId][holder]);
        }
